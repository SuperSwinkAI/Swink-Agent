...
fn registry_with_defaults_has_four_evaluators() {
    ...
    let invocation = crate::common::mock_invocation(&["read", "write"], Some("hello"), 0.01, 500);
    let case =
        crate::common::case_with_trajectory(vec![swink_agent_eval::ExpectedToolCall::new("read")]);
...
fn custom_evaluator_alongside_defaults() {
    ...
    let invocation = crate::common::mock_invocation(&[], Some("hello"), 0.01, 500);
...
fn evaluator_returning_none_excluded() {
    ...
    let invocation = crate::common::mock_invocation(&[], None, 0.0, 0);
    let case = minimal_case();
    let results = registry.evaluate(&case, &invocation);
