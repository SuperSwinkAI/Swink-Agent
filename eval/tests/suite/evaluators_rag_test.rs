...
use crate::common::{mock_invocation, mock_invocation_with_response};
...
fn retrieved_passages() -> Vec<FewShotExample> {
    vec![
        FewShotExample::new("Paris is the capital of France.", "supporting passage"),
        FewShotExample::new("France is a country in Europe.", "supporting passage"),
    ]
}
...
async fn rag_groundedness_records_prompt_version_against_retrieved_context() {
    ...
    let mut case = crate::common::case_with_trajectory(vec![]);
...
fn rag_groundedness_returns_none_without_retrieved_context() {
    ...
    let case = crate::common::case_with_trajectory(vec![]); // no few_shot_examples
...
fn rag_helpfulness_returns_none_without_retrieved_context() {
    ...
    let case = crate::common::case_with_trajectory(vec![]);
    let invocation = mock_invocation_with_response(&[], "answer");
