...
async fn cache_hit_skips_agent_invocation() {
    ...
    let set = set_with(vec![
        crate::common::make_case("c1"),
        crate::common::make_case("c2"),
    ]);
...
async fn changing_user_messages_invalidates_cache() {
    ...
    let mut case_v2 = crate::common::make_case("c1");
    ...
    let _ = runner
        .run_set(&set_with(vec![crate::common::make_case("c1")]), &factory)
        .await
        .unwrap();
    let _ = runner
...
async fn disk_cache_round_trip_survives_restart() {
    let dir = TempDir::new().unwrap();
    let set = set_with(vec![crate::common::make_case("c1")]);
    {
...
fn cache_key_is_deterministic_64_hex() {
    let fp = crate::common::make_case("c1").cache_fingerprint();
...
fn local_file_store_writes_expected_layout() {
    ...
    let invocation = crate::common::mock_invocation(&[], Some("hi"), 0.0, 0);
