...
use crate::common::{case_with_trajectory, mock_invocation, mock_invocation_multi_turn};

#[test]
fn exact_match_passes() {
    let matcher = TrajectoryMatcher::exact();
    let case = case_with_trajectory(vec![
        ExpectedToolCall::new("read"),
        ExpectedToolCall::new("write"),
    ]);
...
fn exact_match_fails_on_extra_tool() {
    let matcher = TrajectoryMatcher::exact();
    let case = case_with_trajectory(vec![ExpectedToolCall::new("read")]);
    let invocation = mock_invocation(&["read", "write"], Some("done"), 0.01, 100);
    let result = matcher.evaluate(&case, &invocation).unwrap();
...
fn us2_arguments_matching_exact_json() {
    ...
    let case = case_with_trajectory(vec![
        ExpectedToolCall::new("read").with_arguments(serde_json::json!({"file": "a.txt"})),
    ]);
