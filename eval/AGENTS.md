...
- `EvalRunner` owns the `CancellationToken` returned by `AgentFactory::create_agent()`. Cancel it on every `invoke_agent_impl()` exit path, including `prompt_stream()` startup failures and runner-cancelled `select!` branches, so factory-specific cleanup wiring always fires when a case stops early.
