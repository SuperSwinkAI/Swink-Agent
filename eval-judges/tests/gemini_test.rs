//! Wiremock-backed integration tests for [`GeminiJudgeClient`].
//!
...
use swink_agent_eval_judges::GeminiJudgeClient;
use tokio_util::sync::CancellationToken;
use wiremock::matchers::{method, path};
use wiremock::{Mock, MockServer, Request, Respond, ResponseTemplate};

mod common;
use common::{gemini_success_body, happy_verdict_json};
...
async fn gemini_happy_path() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(gemini_success_body(&happy_verdict_json()))
        .expect(1)
        .mount(&server)
        .await;

    let client = GeminiJudgeClient::new(server.uri(), "test-key", TEST_MODEL)
...
#[tokio::test]
async fn gemini_rate_limit_absorbed_by_retry() {
    let server = MockServer::start().await;
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(OneBlipThenSuccess {
            count: count.clone(),
            success: gemini_success_body(&happy_verdict_json()),
        })
        .expect(2)
        .mount(&server)
        .await;
...
#[tokio::test]
async fn gemini_exhausted_retries_surface_transport() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn gemini_malformed_response_is_terminal() {
    let server = MockServer::start().await;

    Mock::given(method("POST"))
        .and(path(generate_path()))
        .respond_with(gemini_success_body("not a verdict at all"))
        .expect(1)
        .mount(&server)
        .await;
...
#[tokio::test]
async fn gemini_cancellation_token_short_circuits() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn gemini_blocking_wrapper_delegates_to_async() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path(generate_path()))
        .respond_with(gemini_success_body(&happy_verdict_json()))
    ...
    let client = GeminiJudgeClient::new(server.uri(), "test-key", TEST_MODEL)
        .with_retry_policy(test_policy(3));
    let blocking = swink_agent_eval_judges::BlockingGeminiJudgeClient::new(client);
