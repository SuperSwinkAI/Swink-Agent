//! Wiremock-backed integration tests for [`XaiJudgeClient`].
//!
//! Coverage mirrors the existing OpenAI-compatible judge clients:
//! happy path, retryable 429 recovery, exhausted retries, malformed verdict,
...
use swink_agent_eval_judges::XaiJudgeClient;
...
async fn xai_happy_path() {
    ...
    let client = XaiJudgeClient::new(server.uri(), "test-key", "grok-test")
...
#[tokio::test]
async fn xai_rate_limit_absorbed_by_retry() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn xai_exhausted_retries_surface_transport() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn xai_malformed_response_is_terminal() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn xai_cancellation_token_short_circuits() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn xai_blocking_wrapper_delegates_to_async() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(openai_success_body(&happy_verdict_json()))
    ...
    let client = XaiJudgeClient::new(server.uri(), "test-key", "grok-test")
        .with_retry_policy(test_policy(3));
    let blocking = swink_agent_eval_judges::BlockingXaiJudgeClient::new(client);
