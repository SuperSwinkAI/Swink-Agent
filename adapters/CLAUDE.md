...
- **Bedrock known event payloads are strict** — malformed JSON for known event types (`messageStart`, `contentBlock*`, `messageStop`, `metadata`) must terminate the stream after draining open blocks. Only truly unknown event types are safe to skip.
