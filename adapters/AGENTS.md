...
- **Azure** — Entra token endpoint failures are not all retryable transport errors: classify credential/configuration `4xx` responses (including `400 invalid_client`) as `error_auth(...)`, keep `429` throttled, keep transport/`5xx` failures retryable, and treat malformed token payloads as terminal generic errors.
