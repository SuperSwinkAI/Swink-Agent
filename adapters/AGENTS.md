...
- **Azure** — race Entra token acquisition with the stream `CancellationToken` before entering the shared OAI transport. Otherwise user cancellation can hang behind token refresh HTTP latency and still hit the API server after the run is already aborted.
- **Azure** — Entra token endpoint failures are not all retryable transport errors: classify credential/configuration `4xx` responses (including `400 invalid_client` and wrong-tenant `404`) as `error_auth(...)`, keep `408`/transport/`5xx` failures retryable, keep `429` throttled, and treat malformed token payloads as terminal generic errors.
...
- **Bedrock** — malformed JSON for known event types is a non-retryable protocol fault, so that terminal path must emit `AssistantMessageEvent::error(...)`, not `error_network(...)`.
...
- **Bedrock** — `send_converse_stream()` must also be raced against cancellation in the init state, not just checked beforehand. A plain `is_cancelled()` guard misses cancellations that arrive while the initial signed HTTP request is still waiting on the network.
...
- In `src/ollama.rs`, EOF before any terminal `done: true` chunk is a transport failure, not a protocol success or malformed-payload case. Finalize open blocks, then emit `AssistantMessageEvent::error_network(...)` so loop retry classification stays retryable.
