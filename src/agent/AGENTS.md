...
`src/agent/` — `Agent` internals: event dispatch, lifecycle control (pause/resume/reset), checkpointing, compaction, queueing, structured output. The handle itself is `src/agent.rs`; the registry is `src/registry.rs`.
