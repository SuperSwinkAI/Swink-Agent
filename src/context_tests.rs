...
fn model_with_window(window: u64) -> crate::types::ModelSpec {
    crate::types::ModelSpec {
        ...
        capabilities: Some(crate::types::ModelCapabilities::none().with_max_context_window(window)),
    }
}
...
fn tool_schema_tokens_price_name_description_and_schema() {
    ...
    let expected =
        (tool.name().len() + tool.description().len() + tool.parameters_schema().to_string().len())
            / 4;
