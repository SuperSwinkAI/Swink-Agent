...
MSRV **1.88** (edition 2024). Common workspace deps are centralized in root `Cargo.toml`, with crate-specific dependencies declared locally where needed.
...
- Shared reusable policy implementations primarily live in `swink-agent-policies`; plugin crates can also define crate-local policies against the core traits.
