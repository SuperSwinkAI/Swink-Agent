...
#[tokio::test]
async fn prompt_pre_turn_policies_receive_only_fresh_prompt_batch() {
    let stream_fn = Arc::new(MockStreamFn::new(vec![
        text_only_events("first response"),
        text_only_events("second response"),
    ]));
    ...
    let first = agent.prompt_async(vec![user_msg("hello")]).await.unwrap();
    ...
    let recorded = observations.lock().unwrap().clone();
    assert_eq!(recorded.len(), 2, "policy should run once per prompt");
    assert_eq!(recorded[0], vec!["hello".to_string()]);
    assert_eq!(
        recorded[1],
        vec!["follow up".to_string()],
        "fresh prompt calls should not expose old conversation history as new_messages"
    );
}
