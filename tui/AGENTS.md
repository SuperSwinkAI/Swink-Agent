...
- **Bare `#key` must stop at `hash_key_args()`** — return `None`, not `Some("")`, for argument-less `#key`. Secret detection and command execution should reject it at the shared parser boundary instead of relying on `parse_key_provider_and_value("")` to absorb an invalid intermediate state.
...
- **Session restore must read transcript + state atomically** — `App::load_session()` must use `SessionStore::load_full()` instead of separate `load()` / `load_state()` calls; otherwise a concurrent save can mix transcript and state revisions during resume.
- **Session restore must preserve tool-result error roles** — `App::load_session()` should map saved `LlmMessage::ToolResult` records through `tool_result.is_error`, matching live `TurnEnd` rendering so failed tool outputs do not come back as successful tool messages after reload.
- **`#copy code` must concatenate every fenced block from the last assistant message** — do not stop at the final block. Shared markdown extraction should preserve block order and return `None` only when no fenced blocks exist, so clipboard copies satisfy spec 028's multi-block scenario.
