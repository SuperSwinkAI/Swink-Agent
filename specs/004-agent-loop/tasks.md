...
- [ ] T072 [P] [US6] Write test verifying no-compaction-skip (FR-013d): transformers are configured but both return `None` (no compaction) — verify error is surfaced immediately without retrying the LLM call in `tests/loop_overflow.rs`
- [ ] T073 [P] [US6] Write test verifying cancellation during emergency recovery: cancellation token fires between compaction and retry stream call — verify recovery aborts and loop emits `Aborted` stop reason in `tests/loop_overflow.rs`
...
- [ ] T071 [US6] Deprecate the `CONTEXT_OVERFLOW_SENTINEL` encoding path in `handle_stream_result` — overflow recovery now happens in-place in `run_single_turn`. Mark the sentinel constant with `#[deprecated]` but retain it for backward compatibility. Do not remove.
