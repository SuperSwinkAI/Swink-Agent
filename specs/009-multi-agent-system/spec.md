...
- What happens when two agents are registered with the same name — depends on the layer: the base `AgentRegistry::register` (src/registry.rs) silently replaces the existing entry (documented behavior, see tests/registry.rs `duplicate_name_overwrites`); `AgentOrchestrator::add_agent`/`add_child` (src/orchestrator.rs) instead panics on a duplicate name, since the orchestrator's factory-based bookkeeping assumes unique names per managed agent.
...
- Q: Should duplicate agent name registration replace or reject? → A: [2026-07-06] Split by layer — the base `AgentRegistry` replaces the existing entry (last write wins); the `AgentOrchestrator` panics on a duplicate name, since it manages unique named agent entries with parent/child bookkeeping.
...
- Agent names are unique within an orchestrator: `AgentOrchestrator::add_agent`/`add_child` panic on a duplicate name. The base `AgentRegistry` does not enforce uniqueness — registering a duplicate name silently replaces the prior entry. [2026-07-06]
