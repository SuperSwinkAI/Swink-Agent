...
- **FR-003**: The adapters crate MUST provide an `all` feature that enables all 9 adapter feature flags (5 implemented + 4 stubs), plus a `full` convenience feature for the all-adapters profile. **Corrected 2026-09-14 (#1313)**: `all` now enables 11 provider features (the original 9 plus `responses` and `codex`).
- **FR-004**: The local-llm crate MUST expose backend feature flags (`metal`, `cuda`, `cudnn`, `vulkan`) that forward to the corresponding llama-cpp-2 compile-time features. **Corrected 2026-07-06**: this list previously omitted `cudnn` (verified against `local-llm/Cargo.toml`: `cudnn = ["cuda"]`). **Addendum 2026-09-14 (#1313)**: local-llm also exposes a non-backend `gemma4` feature that gates the Gemma 4 model presets.
