...
- [x] T001 [P] [US1] Add `[features]` section to `adapters/Cargo.toml` with 9 individual adapter flags (`anthropic`, `openai`, `ollama`, `gemini`, `proxy`, `azure`, `bedrock`, `mistral`, `xai`), `all` aggregator, and `default = ["all"]` **[corrected 2026-07-06: the actual shipped default is `default = []`, matching spec.md:107 — `default = ["all"]` as written here was never the final choice]**. The `gemini` feature gates the `google` module (file is `google.rs`, public type is `GeminiStreamFn`). Make `eventsource-stream` optional and gate behind `proxy` feature (`proxy = ["dep:eventsource-stream"]`). Make `sha2` optional and gate behind `bedrock` feature (`bedrock = ["dep:sha2"]`). **[corrected 2026-07-06: neither `eventsource-stream` nor `sha2` appears in `adapters/Cargo.toml`. `proxy = []` — the proxy adapter's SSE handling is a hand-rolled, always-on module with no dependency of its own. `bedrock`'s real optional deps are `aws-credential-types`, `aws-sigv4`, `aws-smithy-eventstream`, `aws-smithy-runtime-api`, `aws-smithy-types`.]**
...
- Provider-specific deps become optional, gated by their provider feature — **[corrected 2026-07-06: not `eventsource-stream`/`sha2` as originally written; the real gated deps are bedrock's `aws-credential-types`/`aws-sigv4`/`aws-smithy-eventstream`/`aws-smithy-runtime-api`/`aws-smithy-types` family. Proxy has no optional dependency of its own — see T001's correction.]**
