...
- Q: Should PolicyContext include conversation messages? → A: Yes. Added `new_messages: &[AgentMessage]` to PolicyContext (FR-010) — contains only messages added since the last evaluation for this slot. For PreTurn: the pending batch (user messages, injected messages). For PostTurn/PostLoop/PreDispatch: empty (current-turn data is in TurnPolicyContext/ToolPolicyContext). Avoids redundant re-scanning of already-evaluated messages. Required by 032-policy-recipes-crate (PromptInjectionGuard). Backward-compatible.
