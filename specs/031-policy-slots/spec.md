...
- **FR-010**: Every policy evaluation MUST receive a shared context containing: turn index, accumulated usage, accumulated cost, message count, overflow signal, and a read-only slice of new messages since the last evaluation (`new_messages: &[AgentMessage]`). For PreTurn, this is the pending batch appended since the previous turn. For PostTurn and PostLoop, this is empty — current-turn data is available via `TurnPolicyContext`. **Corrected 2026-07-06**: PreDispatch does not receive this shared context at all — the shipped `PreDispatchPolicy::evaluate` takes only `ToolDispatchContext` (real name; this doc previously called it `ToolPolicyContext`), because loop-level metrics are not tracked at the tool-dispatch call site.
- **FR-011**: PreDispatch policies MUST receive per-tool-call context with tool name, tool call ID, mutable access to arguments, and (corrected 2026-07-06, matching the shipped `ToolDispatchContext`) the tool's `execution_root` and read-only `state` — not loop-level metrics.
...
- **FR-018**: The slot runner MUST catch panics from policy evaluate methods via `catch_unwind` (using `AssertUnwindSafe` wrapper), treat panics as Continue (skip the panicking policy), and log at warn level with the policy name. Panicking policies MUST NOT crash the agent loop. For `PreDispatch` policies specifically, tool arguments are snapshotted before each policy runs; if a policy panics, the snapshot is restored before subsequent policies or approval see the arguments — ensuring no partial argument mutations persist after a panic.
- **FR-019**: Built-in policy implementations (BudgetPolicy, MaxTurnsPolicy, SandboxPolicy, ToolDenyListPolicy, CheckpointPolicy, LoopDetectionPolicy) MUST be provided as opt-in convenience structs. None MUST be enabled by default. Amended by [#1070](https://github.com/SuperSwinkAI/Swink-Agent/issues/1070), which adds `RollingCheckpointPolicy` (a single-overwritten-checkpoint variant of CheckpointPolicy for long-session crash-safety) under the same rule: opt-in, never enabled by default. Default-on crash-safety checkpointing was considered and rejected — persistence stays an embedder decision (see the 2026-07-15 clarification below).
...
- **PolicyContext**: Shared read-only context for all policy evaluations — turn state, accumulated usage, accumulated cost, message count, overflow signal, and new messages since last evaluation.
- **ToolDispatchContext** (named `ToolPolicyContext` in earlier drafts of this doc — corrected 2026-07-06): Per-tool-call context with mutable access to arguments, plus `execution_root` and read-only session `state`; available to PreDispatch policies in place of (not alongside) the shared `PolicyContext`.
...
- Q: Should PolicyContext include conversation messages? → A: Yes. Added `new_messages: &[AgentMessage]` to PolicyContext (FR-010) — contains only messages added since the last evaluation for this slot. For PreTurn: the pending batch (user messages, injected messages). For PostTurn/PostLoop: empty (current-turn data is in TurnPolicyContext). Avoids redundant re-scanning of already-evaluated messages. Required by 032-policy-recipes-crate (PromptInjectionGuard). Backward-compatible. **Corrected 2026-07-06**: PreDispatch was never one of the "empty PolicyContext" cases — it doesn't receive a `PolicyContext` at all, only `ToolDispatchContext` (this doc previously called it `ToolPolicyContext`).

### Session 2026-07-15 (checkpoint hardening, [#1070](https://github.com/SuperSwinkAI/Swink-Agent/issues/1070))

- Q: Should CheckpointPolicy be enabled by default for crash safety? → A: No. FR-019/FR-009 stand unchanged: the empty-slots default remains anything-goes, and silently persisting full conversation history to disk would be the most surprising default the library could acquire. Crash-safety default-on belongs at the embedder layer. Instead, the opt-in path was hardened.
- Q: How do checkpoint IDs stay distinct across multiple `prompt()` runs? → A: `CheckpointPolicy::with_session_id` (additive builder) scopes IDs to `"{session}-turn-{n}"`. Without it, IDs are `"turn-{n}"` and the per-run turn index makes a second run overwrite the first run's checkpoints — restoring "the highest turn" can then resurrect stale history. The unscoped format is retained as the default for backward compatibility; the hazard is documented on the builder.
- Q: What about long sessions, where per-turn full-history snapshots cost O(N²) bytes? → A: New opt-in `RollingCheckpointPolicy` (same `checkpoint` feature, same fire-and-forget spawn contract as FR-024) overwrites one stable ID — optionally `"{session}-rolling"` — via the store's existing atomic save path. Disk cost is O(context); at most one turn is lost on crash; no time-travel. Complemented by `FileCheckpointStore::with_max_checkpoints(n)` retention in `swink-agent-memory` (default remains unlimited; pruning only ever deletes files that parse as checkpoints).
