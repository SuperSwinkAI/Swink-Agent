...
- [x] T056 [P] Write integration test: `post_loop_policy_can_stop_outer_loop` — PostLoop returns Stop, verify loop exits before follow-up poll — **[covered 2026-09-12 in `tests/suite/policy_slots.rs::post_loop_stop_exits_before_queued_follow_up_runs`, which queues a follow-up before `prompt_async` and verifies PostLoop::Stop exits before a second stream call]**
