...
**The `libc` exception to unsafe-free**: `CommandExt::pre_exec` plus `libc::setrlimit`, `libc::unshare`, `libc::waitpid`, and `libc::kill` require unsafe blocks. They are wrapped in a safe `evaluators::code::sandbox::posix` submodule whose internal blocks use `// SAFETY:` comments explaining the invariants. This is the only FFI surface in the eval crates; the module compiles only on `cfg(target_family = "unix")`, and `unsafe_code` is relaxed only at that submodule to `#![allow(unsafe_code)]` with a module-level explanation. Because Rust `forbid` lints cannot be relaxed by nested `allow`, the `swink-agent-eval` crate root and `Cargo.toml` use `deny(unsafe_code)` so every unsafe use outside that one module remains a hard compile error. This exception is explicitly authorized by FR-049 in spec.md (see the exception clause pinning the module path, the `cfg` gate, and the `// SAFETY:` requirement).
