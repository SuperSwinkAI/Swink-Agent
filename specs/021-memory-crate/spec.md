...
FR-004 is extended (**FR-004a**, implemented 2026-07-12): session load MUST tolerate an invalid-UTF-8 (or otherwise unparseable) line — in particular a multi-byte UTF-8 sequence truncated at the tail by a crash mid-append — the same way it already tolerates a malformed-but-valid-UTF-8 line: log a warning and skip that one line rather than failing the whole load. This applies to both the full forward scan and the windowed meta tail scan, which must agree. The metadata line (line 1) remains strict: if it is unreadable, the session is unrecoverable and the load fails with `InvalidData`.
