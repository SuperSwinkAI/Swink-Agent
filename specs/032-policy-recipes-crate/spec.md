...
- **FR-002**: Each policy MUST be independently feature-gated (`prompt-guard`, `pii`, `content-filter`, `audit`, `memory-nudge`) with an `all` feature that enables everything, defaulting to `all`.
  - **Addendum (2026-07-06)**: Post-implementation, commit `3a4d54d` consolidated *all* policies — including 031-policy-slots' six built-ins (`budget`, `max-turns`, `deny-list`, `sandbox`, `loop-detection`, `checkpoint`) — into this crate rather than leaving them in core. `swink-agent-policies`'s `all` feature now gates 11 policies, not the 5 originally scoped here: `budget`, `max-turns`, `deny-list`, `sandbox`, `loop-detection`, `checkpoint`, `prompt-guard`, `pii`, `content-filter`, `audit`, `memory-nudge` (verified against `policies/Cargo.toml`). This spec's scope (User Stories 1-5, FR-004 through FR-023) still only covers the original 4 application-level policies plus `MemoryNudgePolicy`; the six structural policies are covered by 031-policy-slots and now live here as an implementation detail, not a 032 requirement.
