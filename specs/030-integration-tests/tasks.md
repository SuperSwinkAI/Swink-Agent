...
- [x] T036 [US5] Implement AC 25 test `proxy_stream_reconstruction` in `tests/ac_structured.rs` — create a `ProxyStreamFn`, feed it serialized event data (Start, TextDelta, Done), stream the events, and assert the reconstructed event sequence matches the original — **[corrected 2026-07-06: the shipped `proxy_stream_reconstruction` (`tests/ac_structured.rs:109`) does not construct a `ProxyStreamFn` at all — it calls `accumulate_message()` directly on a hand-built event vector, so it exercises message accumulation, not the proxy adapter. Real `ProxyStreamFn` reconstruction-over-HTTP/SSE coverage lives in `adapters/tests/proxy_http.rs` (`successful_stream_reconstructs_message`, `text_and_tool_call_reconstruct_correctly`), which was not written as part of this task.]**
