...
- **FR-003**: System MUST provide a built-in in-memory credential store, seeded at agent instantiation via configuration. This is the only store enabled by default; consumers may implement the trait for custom backends.
- **FR-003a**: System MAY ship additional credential store implementations behind opt-in feature flags, provided they are off by default and nothing in the default build depends on them. Amended by [#1068](https://github.com/SuperSwinkAI/Swink-Agent/issues/1068), which adds `KeychainCredentialStore` behind the `keychain` feature of `swink-agent-auth`. Such stores are a convenience for embedders; the framework still never selects or configures persistent storage on a consumer's behalf (see FR-019 and the Clarifications note below).
...
- **CredentialStore**: A pluggable storage trait for credentials. Supports get/set/delete by string key. The framework ships an in-memory implementation by default and an optional keychain-backed one behind the `keychain` feature (FR-003a); consumers may implement the trait for custom backends.
...
- **SC-008**: The device code flow (RFC 8628) can be completed end-to-end (device code request through polling to token storage) using a configurable handler, with the polling loop honoring `authorization_pending`, `slow_down` back-off, and terminal errors. ([#1071](https://github.com/SuperSwinkAI/Swink-Agent/issues/1071))
...
### Session 2026-07-14 — keychain store, partially reversing 2026-03-31

- Q: [#1068](https://github.com/SuperSwinkAI/Swink-Agent/issues/1068) asks for a keychain-backed `CredentialStore`, which the 2026-03-31 clarification dropped as FR-005. Is this a reversal? → A: **Partially — the scope differs.** The 2026-03-31 decision rejected *framework-managed* persistence: a store the framework selects, chains, or falls back to on a consumer's behalf. What it was protecting against is implicit behavior (silently reading the OS keychain, a chain that makes resolution order ambiguous) and a dependency load that headless deployments cannot pay. #1068 asks for something narrower: **one more implementation of the public trait, behind an off-by-default feature flag, that a consumer must explicitly construct and pass in.** Nothing resolves through it unless the embedder names it. That does not reintroduce framework-managed persistence, so the original decision's intent survives intact.
- Q: What specifically stays dropped? → A: Env var store (FR-004) and store chaining (FR-006) remain dropped, and neither is reconsidered here — chaining in particular is what made resolution order ambiguous. The keychain store is **not** re-listed as FR-005; it is admitted under the new FR-003a as an optional extra, not a required built-in. With default features, `swink-agent-auth` keeps the same dependency graph and public API it had before #1068.
- Q: Why relax it now, given SuperSwink-Core already bridges this itself? → A: Precisely because every embedder currently hand-rolls the same `CredentialStore` impl to survive a restart with OAuth2 refresh tokens intact (FR-021). Shipping one correct, secret-hygienic implementation is cheaper than each consumer rediscovering the same pitfalls. #1068 records it as a nice-to-have, not a blocker, and the feature gate keeps the cost at zero for anyone who does not opt in.

## Assumptions

- OAuth2 authorization code flow was the only supported OAuth2 grant type in the initial implementation. The device code flow (RFC 8628) was added later by [#1071](https://github.com/SuperSwinkAI/Swink-Agent/issues/1071) as a *parallel* seam (`DeviceCodeHandler`/`DeviceAuthorizationConfig`) rather than an extension of the authorization-code types: `AuthorizationHandler::authorize` must return an authorization code, which has no device-flow equivalent (the resolver polls instead), and `AuthorizationConfig` requires a `redirect_uri` the device flow does not use. The shared resolution machinery (single-flight deduplication, authorization timeout, credential storage, error variants) was reused unchanged, so the original "should not preclude" assumption held at the plumbing level but not at the level of the two public authorization-code types. Client credentials flow remains out of scope; it should follow the same parallel-seam pattern.
- The authorization handler is a simple callback that receives a URL and returns an authorization code. The framework does not embed a web server or browser automation — these are the handler's responsibility. The device-code handler is display-only: it receives a `DeviceCodePrompt` (user code and verification URI, never the polling secret) and returns nothing.
- The framework does not manage credential persistence. Credentials are passed in at agent instantiation, and the only store enabled by default is in-memory. Consumers who need persistent storage (env vars, secrets manager) implement the `CredentialStore` trait themselves. As of [#1068](https://github.com/SuperSwinkAI/Swink-Agent/issues/1068) the `keychain` feature of `swink-agent-auth` ships one such implementation (`KeychainCredentialStore`) so the common case need not be hand-rolled — but the consumer still constructs it and passes it in explicitly (FR-003a).
- **Terminology note**: "Credential" here means *tool authentication* secrets (API keys, bearer tokens, OAuth2 tokens for tools an agent calls), resolved via `CredentialStore`/`CredentialResolver`. This is unrelated to the TUI's keychain-backed storage of *LLM provider* API keys (`tui/src/credentials.rs`) — the two systems share a name but nothing else. As of [#1068](https://github.com/SuperSwinkAI/Swink-Agent/issues/1068) this crate also has an optional keychain-backed `CredentialStore` (`keychain` feature), so the two now share the `keyring` crate as well — but they remain distinct systems with different service names, and neither reads the other's entries.
