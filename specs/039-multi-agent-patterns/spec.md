...
- **FR-011**: All pipeline types MUST resolve agents by name from an `AgentFactory` at execution time, not at pipeline construction time. (Pivoted from the originally planned `AgentRegistry` coupling — see plan.md's "AgentFactory instead of AgentRegistry coupling" and research.md R1: `Agent`/`AgentOptions` are not `Clone`, so the executor needs a factory to produce fresh instances per step.)
