...
- [x] T080 Add `swink-agent-policies` as a dependency in `eval/Cargo.toml` (path dependency within the workspace). Remove `tokio-util` from `eval/Cargo.toml` if it becomes unused after Phase 13 completes (verify via `cargo check -p swink-agent-eval` after T086 lands). **[Verified 2026-07-06: `tokio-util` retained — still used by runner/evaluator/judge for `CancellationToken`.]**
- [x] T081 Reshape `BudgetConstraints` in `eval/src/types.rs`: drop fields `max_tokens: Option<u64>` and `max_duration: Option<Duration>`; add `max_input: Option<u64>` and `max_output: Option<u64>`. Keep `max_cost: Option<f64>` and `max_turns: Option<usize>`. Update the struct's rustdoc to reference `BudgetPolicy` / `MaxTurnsPolicy`.
- [x] T082 Add `BudgetConstraints::to_policies(&self) -> (Option<BudgetPolicy>, Option<MaxTurnsPolicy>)` in `eval/src/types.rs`: returns `(None, None)` when all fields are `None`; constructs `BudgetPolicy` from cost/input/output fields when any is set; constructs `MaxTurnsPolicy` from `max_turns` when set. Unit-test all four permutations inline.
- [x] T083 [P] Update `eval/src/efficiency.rs` line 219 and any inline tests that construct `BudgetConstraints` with the removed fields. `FR-005`'s `ideal_turns` derivation from `budget.max_turns` is unchanged.
- [x] T084 [P] Update `eval/tests/efficiency.rs` test cases to use the reshaped `BudgetConstraints` (drop `max_tokens`/`max_duration`, add `max_input`/`max_output` where relevant).
- [x] T085 [P] Update `eval/tests/common/mod.rs::case_with_budget()` helper signature to accept the reshaped `BudgetConstraints`. Callers in tests using dropped fields are updated in sibling tasks (T084, T087).
- [x] T086 Refactor `eval/src/runner.rs`: remove `BudgetGuard::from_case` call and `collect_with_guard` invocation. Replace with `collect_from_stream`. Remove the `BudgetGuard` import, the `CancellationToken` construction tied to the guard, and any guard-specific wiring. Document in the `AgentFactory` trait rustdoc that implementers MUST attach `case.budget.to_policies()` to the agent via `AgentOptions::with_pre_turn_policy(...)` if budget enforcement is desired.
- [x] T087 Delete the `BudgetGuard` struct, `impl BudgetGuard`, `TrajectoryCollector::exceeds_budget`, `TrajectoryCollector::collect_with_guard`, and the `sleep_until_deadline` helper from `eval/src/trajectory.rs`. Remove all inline `#[cfg(test)]` tests that exercise `BudgetGuard` (guard-specific tests in `eval/src/trajectory.rs` lines ~355–505). Clean up now-unused imports (`CancellationToken`, `tokio_util`, `Duration` if no longer used).
- [x] T088 Rewrite `eval/tests/budget.rs`: remove all tests that construct `BudgetGuard` directly. Keep tests that exercise `BudgetEvaluator` (evaluator-side budget scoring, not mid-stream cancellation). Add a new integration test `budget_policy_stops_multi_turn_run` that constructs an `AgentFactory` attaching `BudgetPolicy::new().max_cost(1.0)` to the agent, runs a multi-turn case, and asserts `Invocation.stop_reason` reflects the policy-stop verdict. Add `budget_constraints_to_policies_none_when_unset` verifying `BudgetConstraints::default().to_policies() == (None, None)`.
- [x] T089 Remove `BudgetGuard` from `eval/src/lib.rs` re-exports. The `pub use trajectory::{BudgetGuard, TrajectoryCollector};` line becomes `pub use trajectory::TrajectoryCollector;`.
- [x] T090 Replace T011's superseded test: in `eval/tests/budget.rs` (or equivalent agent-loop integration test), verify that attaching `BudgetPolicy::new().max_cost(0.01)` to a multi-turn run causes the loop to stop with a `PolicyVerdict::Stop` reason and `Invocation` is still complete with finalized turn records. This exercises the agent-loop equivalent of T011's "cancels on budget breach" contract (spec AS-1.x carries over — trajectory remains complete).
- [x] T091 Replace T012's superseded test: in `eval/src/types.rs` inline tests, verify `BudgetConstraints::default().to_policies() == (None, None)`, and that `BudgetConstraints { max_cost: None, max_input: None, max_output: None, max_turns: None }.to_policies() == (None, None)`.
...
- [x] T093 Verify `cargo test -p swink-agent-eval` and `cargo clippy -p swink-agent-eval -- -D warnings` both pass after migration. **[Verified 2026-07-06: workspace-wide `clippy --all-targets -- -D warnings` and full test suite green.]**
- [x] T094 Verify `cargo test --workspace` — no regressions in other crates (particularly `runner.rs` consumers in integration tests). **[Verified 2026-07-06: 2,799 tests passed, 0 failed.]**
