...
An evaluator wants to measure how efficiently the agent solved a problem — did it take unnecessary steps or repeat the same tool calls? The efficiency evaluator computes a score based on two factors: the ratio of duplicate invocations (weighted at 0.6) and the ratio of actual steps to expected steps (weighted at 0.4). A perfect score means no duplicates and exactly the expected number of steps. The evaluator uses this score to compare different models or prompts on the same task.
...
5. **Given** multiple criteria combined, **When** all pass, **Then** the overall match succeeds; when any fails, the overall match fails with details of which criterion failed.

---

### Edge Cases

- What happens when the golden path is empty (no expected steps)?
- How does the trajectory matcher handle tool calls with identical names but different inputs — are they considered the same step?
- What happens when the agent produces an extremely long trajectory (thousands of invocations)?
- How does efficiency scoring handle a golden path with zero expected steps?
- What happens when a regex criterion contains an invalid regular expression?
- How does the trajectory collector handle concurrent tool executions that complete out of order?
- What happens when the custom matching function panics?
- How does the matcher handle partial matches (e.g., correct tool name but wrong input)?
...
- **FR-003**: The system MUST compare an actual trajectory against a golden path, identifying matched, missing, and unexpected steps.
- **FR-004**: The system MUST report ordering deviations between the actual trajectory and the golden path.
- **FR-005**: The system MUST compute an efficiency score using the formula: (1 - duplicate_ratio) * 0.6 + (expected_steps / actual_steps) * 0.4, clamped to [0.0, 1.0].
...
- **FR-007**: The system MUST support combining multiple response criteria, reporting which specific criteria failed.
...
- **TrajectoryCollector**: Observes the agent event stream and builds a structured list of invocation records, one per tool call.
- **Invocation**: A single tool call record containing the tool name, inputs, result, success/failure status, and timing information.
- **TrajectoryMatcher**: Compares an actual trajectory against a golden path (expected invocation sequence), producing a comparison report of matched, missing, and unexpected steps.
